...
Mac/802_11Ext set dataRate_ 5Mb				;# 选择802_11Ext协议，并设置数据发送速率为6Mb/s
...
set tracefd 			[open ns_trace.tr w]	;# 定义.tr的trace文件，运行结束得到ns_trace.tr
...
for {set i 0} {$i < [expr $val(nn)+$val(nr)]} {incr i} {
	...
	$agent_($i) set payloadSize 4352		;# 分组大小
	$agent_($i) set periodicBroadcastInterval 0.2 ;# 广播周期为200毫秒
